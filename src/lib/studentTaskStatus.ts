...
/** Dropdown order: incomplete-style states first, then complete, then missing/excused. */
...
export function studentTaskStatusSelectClass(status: StudentTaskStatus): string {
  const map: Record<StudentTaskStatus, string> = {
    ...
    excused:
      "border-violet-500/50 bg-violet-500/10 text-violet-900 dark:border-violet-500/40 dark:bg-violet-950/40 dark:text-violet-100",
  };
  return map[status];
}
...
export function studentTaskStatusItemClass(status: StudentTaskStatus): string {
  const map: Record<StudentTaskStatus, string> = {
    ...
    excused: "focus:bg-violet-500/10",
  };
  return map[status];
}
...
export function studentTaskStatusBadgeClass(status: StudentTaskStatus): string {
  const map: Record<StudentTaskStatus, string> = {
    ...
    excused:
      "border-violet-200 bg-violet-100 text-violet-900 dark:border-violet-800 dark:bg-violet-950/50 dark:text-violet-100",
  };
  return map[status];
}
